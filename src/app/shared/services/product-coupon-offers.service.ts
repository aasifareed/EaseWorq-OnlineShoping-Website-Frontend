...
export class ProductCouponOffersService {
  constructor(
    private http: HttpClient,
    private tenantService: TenantService
  ) {}

  getForProduct(productId: string): Observable<ProductCouponOffer[]> {
    ...
    return this.tenantService.whenReady().pipe(
      switchMap((ctx) => {
        const path = environment.urls.OnlineShopCoupon_GetProductCoupons
  ...
  private mapOffer(row: Record<string, unknown>): ProductCouponOffer | null {
    ...
    return {
      ...
      benefitLabel: this.benefitLabel(type, Number.isFinite(amount) ? amount : null)
    };
  }

  private benefitLabel(type: string, amount: number | null): string {
